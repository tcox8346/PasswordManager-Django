...
class FriendList(models.Model):
    ...
    def b_is_friend(self, user_profile):
        ...
        if user_profile in self.friends_list:
    ...
    def save_base(self, raw: bool = ..., force_insert: bool = ..., force_update: bool = ..., using: str | None = ..., update_fields: Iterable[str] | None = ...):
        if self.slug == '':
...
class FriendRequest(models.Model): # Server Based Encryption
    ...
    def process_request(self, is_accepted:bool, current_user_profile:SolutionUserProfile):
        ...
        if type(is_accepted) == bool and current_user_profile == self.request_target:
            ...
            recipient_friends_list:FriendList= FriendList.objects.filter(owner_profile = self.request_target) 
            requester_friends_list:FriendList = FriendList.objects.filter(owner_profile = self.requester) 
            ...
            # If no errors have occured save the changes to the request record
